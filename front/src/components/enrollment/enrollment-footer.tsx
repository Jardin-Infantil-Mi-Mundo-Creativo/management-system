...
import { INSTITUTION_ADDRESS, INSTITUTION_EMAIL, INSTITUTION_PHONE, INSTITUTION_CELLPHONE } from '@/consts/shared';
...
function EnrollmentFooter({ errors, isEnrollmentMutationPending }: EnrollmentFooterProps) {
  return (
    <>
      <div className='flex flex-col items-center text-sm font-semibold'>
        <p>{`Dirección: ${INSTITUTION_ADDRESS}`}</p>
        <p>Correo: <span className='underline'>{INSTITUTION_EMAIL}</span></p>
      ...
      <div className='flex flex-col items-center mt-6 gap-2'>
        ...
        <Button type="submit" className="w-full" size="lg" disabled={isEnrollmentMutationPending}>
          {isEnrollmentMutationPending ? <Spinner /> : null} {isEnrollmentMutationPending? "Matriculando..." : "Matricular estudiante"}
        </Button>
      </div>
    </>
  )
